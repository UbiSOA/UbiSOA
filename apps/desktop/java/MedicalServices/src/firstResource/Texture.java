 ...
 * 
 */
public class Texture extends BaseResource {
	
	static String textureURI    = "http://localhost:2122/ubicomp/presentation/texture";
	...
	//Variable que contendra el valor de las imagenes de la textura
	static String varImage;
	
    public Texture(Context context, Request request, Response response) {
    ...
    public Representation represent(Variant variant) throws ResourceException {
    	...
    	Property image= model.createProperty(nm, "image");
 		...
 		//Se crea el recurso
 		Resource texture = model.createResource(textureURI);
 		
 		//Se asigna el valor a la variable
 		varImage = "path de la imagen del menu";
 			
 		//Se agregan las propiedades
 		texture.addProperty(image, varImage = "path de la imagen del menu");
 		texture.addProperty(image, "path de la imagen2");
 		texture.addProperty(image, "path de la imagen3");
        ...
        if (MediaType.TEXT_XML.equals(variant.getMediaType())) {
            try {
            	...
            	d.appendChild(nodeUbicomp);
            		...
            		Element nodeTexture = d.createElement("texture");
