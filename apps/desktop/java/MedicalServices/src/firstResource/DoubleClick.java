 ...
 * 
 */
public class DoubleClick extends BaseResource {
	
	static String doubleClickURI    = "http://localhost:2122/ubicomp/event/click";
	...
	//Variables para el file y URL
	static String varFile;
	static String varURL;
	
    public DoubleClick(Context context, Request request, Response response) {
    ...
    public Representation represent(Variant variant) throws ResourceException {
    	...
    	Property url= model.createProperty(nm, "url");
 		...
 		//Se crea el recurso
 		Resource eventDoubleClick = model.createResource(doubleClickURI);
 		
 		//Se asignan los valores a las variables
 		varFile =  "http://usuario.cicese.mx/~jalvarez/cmu/l1.pdf";
 		varURL = "www.cicese.mx";
 			
 		//Se agregan las propiedades
 		eventDoubleClick.addProperty(file,varFile);
 		eventDoubleClick.addProperty(url,varURL);
        ...
        if (MediaType.TEXT_XML.equals(variant.getMediaType())) {
            try {
                ...
                d.appendChild(nodeUbicomp);
                	...
                	Element nodeDoubleClick = d.createElement("doubleClick");

                	Element eltName = d.createElement("file");
                	eltName.appendChild(d.createTextNode(varFile));
                	nodeDoubleClick.appendChild(eltName);

                    Element eltDescription = d.createElement("url");
                    eltDescription.appendChild(d.createTextNode(varURL));
                    nodeDoubleClick.appendChild(eltDescription);

                    nodeEvent.appendChild(nodeDoubleClick);
